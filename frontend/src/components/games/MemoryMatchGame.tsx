...
const { letter } = useParams();
  ...
  const initializeGame = () => {
    const selectedPairs = alefPairs.slice(0, 6);
  ...
  const checkMatch = (flipped: number[]) => {
    ...
    setTimeout(() => {
      if (
        (firstCard.type === 'letter' && secondCard.type === 'word' && secondCard.content.startsWith(firstCard.content)) ||
        (secondCard.type === 'letter' && firstCard.type === 'word' && firstCard.content.startsWith(secondCard.content))
      ) {
        // Match found!
        setCards(prev => prev.map(card => 
        ...
        const allMatched = cards.filter(c => c.id !== first && c.id !== second).every(c => c.matched);
        if (allMatched) {
          ...
          <div className="grid grid-cols-3 md:grid-cols-4 gap-4">
            {cards.map((card, index) => (
              <motion.button
                ...
                style={{
                  backgroundColor: card.flipped || card.matched ? '#ffffff' : '#652b82',
